...
def get_db_chain():
    if 'db_chai' not in st.session_state:
        ...
        OPENAI_CHAT_MODEL = st.secrets["OPENAI_CHAT_MODEL_40"]
        MODEL_DEPLOYMENT_NAME = st.secrets["MODEL_DEPLOYMENT_NAME_40"]
        ...
        llm = AzureChatOpenAI(temperature=0.2, deployment_name=MODEL_DEPLOYMENT_NAME, model=OPENAI_CHAT_MODEL, verbose=True)
